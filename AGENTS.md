...
Add coverage in `test/*.test.ts` using the built-in Node test runner. Validate upload auth, encrypted token handling, storage selection, referer policy selection, non-image passthrough, and proxied image transformation behavior. For manual checks, run `pnpm dev`, upload through `PUT /upload` with a `file` field and optional `storage_id`, then verify `GET /:storageId/:token/:seoFilename` for both raw files and transformed images.
...
Never commit real credentials or production bucket identifiers. Keep `UPLOAD_BEARER_TOKEN`, `TOKEN_SECRET`, and storage provider credentials in Wrangler secrets, start from `wrangler.example.jsonc`, and keep `REFERER_ALLOWLIST`, `STORAGE_CONFIGS_JSON`, and `IMAGE_PROXY_WATERMARK_SCHEMES_JSON` as structured JSON values instead of stringified blobs. Review cache keys carefully when changing watermark, proxy, or referer policy behavior.
